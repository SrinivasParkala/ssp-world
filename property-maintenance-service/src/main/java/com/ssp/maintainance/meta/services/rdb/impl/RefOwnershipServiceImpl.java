...
import com.ssp.maintainance.meta.repositories.OwnershipTypeRepository;
import com.ssp.maintainance.meta.services.AbstractService;
import com.ssp.maintainance.meta.services.RefTypeService;

@Service
public class RefOwnershipServiceImpl extends AbstractService implements RefTypeService<RefOwnershipType> {

	private OwnershipTypeRepository t;

	@Autowired
	public RefOwnershipServiceImpl(OwnershipTypeRepository t) {
		this.t = t;
	}

	@Override
	public List<RefOwnershipType> getAllRefType(MasterControl control) throws InvalidInputException {
		control.addToControl(MasterControl.CURRENT_OBJECT, null);
		preExecute(control, "getAllRefType");
		
		List<RefOwnershipType> refDatas = new ArrayList<>();
		Iterator<RefOwnershipType> iterator = t.findAll().iterator();
	...
	@Override
	public RefOwnershipType getRefTypeById(MasterControl control, RefEntityIntKey id) throws InvalidInputException {
		control.addToControl(MasterControl.CURRENT_OBJECT, null);
		preExecute(control, "getRefTypeById");
		
		Optional<RefOwnershipType> optional =  t.findById(id);
		
		postExecute(control, "getRefTypeById");
		RefOwnershipType refStatusType = null;
	...
	@Override
	public RefOwnershipType saveOrUpdateRefType(MasterControl control, RefOwnershipType refData) throws InvalidInputException {
