...
public class OAuth2Config extends AuthorizationServerConfigurerAdapter {
	...
	@Value("${user.oauth.tokenValiditySeconds}")
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		clients.inMemory().withClient(clientid).secret(clientSecret).scopes(outhRoles.split(",")).redirectUris(redirectionUrl)
					.accessTokenValiditySeconds(Integer.valueOf(tokenValiditySeconds)).refreshTokenValiditySeconds(Integer.valueOf(tokenValiditySeconds));
	}
